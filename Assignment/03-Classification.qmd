...
ha.output.cat.categories = [1,0] # make 1 be "at risk"
...
```{python, version = "python"}
preds = model_final.predict(ha[["?????"]], ha["output"])

fpr, tpr, thresholds = metrics.roc_curve(ha["output"], preds)
...
```{python, version = "python"}
cross_val_score(my_pipeline, ha[["????"]], ha["output"], scoring="roc_auc", cv=cross_validation)
```
...
ha_validation.output.cat.categories = [1,0] # make 1 be "at risk"
